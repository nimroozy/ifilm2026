...
interface AuthContextType {
  ...
  login: () => void;
  logout: () => void;
  user: { name: string; username: string; branch: string; package: string; expiration: string } | null;
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const mockUser = { name: 'Ahmad Karimi', username: 'mobin_user_001', branch: 'Kabul', package: 'Premium 50Mbps', expiration: '2025-03-15' };

  return (
    <AuthContext.Provider value={{ isLoggedIn, login: () => setIsLoggedIn(true), logout: () => setIsLoggedIn(false), user: isLoggedIn ? mockUser : null }}>
      {children}
